...
*Traditional spatial data tends to confuse topology and geometry.*  This is the major motivating problem behind my interest in these ideas. 
...
We can think of this another way, and in computer graphics and in `mesh3d` we store
...
We can store polygons this way too, but converting them to triangles is non-trivial. 
...
A classic **computer graphics** data model called *mesh3d*, it's not widely used but is very powerful. You can visualize a mesh3d with `shade3d()`, all the aesthetics, material properties, geometry and topology are present in the model with some control allowed by the shade3d function. 
...
rglwidget()
rgl.clear()
quads3d(t(qm$vb)[qm$ib,], col = rep(c("firebrick", "dodgerblue")[qm$ib %% 2 + 1], each = 4))
rglwidget()
```
...
The single most confusing thing I found with `mesh3d` was *homogeneous coordinates*. This
is a fourth value on each vertex, making it `X, Y, Z, H` and because it's stored transpose, the `ib` and the `vb` matrices have the same number of rows (4). The `H` loosely corresponds to "zoom", for our purposes set `H = 1`. (If set to 0 no data will
be visible, and a package of mine had this bug until last week.)
